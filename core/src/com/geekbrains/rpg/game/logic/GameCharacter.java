...
public abstract class GameCharacter implements MapElement {
    ...
    protected int[] expTable = {1000, 2000, 4000, 8000, 16000};
    protected int expCount = 0;
    protected int charLevel = 1;
    protected int maxLevel = expTable.length;
...
//                    target.takeDamage(this, weapon.generateDamage());
                    this.successAttack(target, weapon.generateDamage());
                }
    ...
    public void addExp(int amount) {
        expCount += amount;
        if(charLevel < maxLevel && expCount >= expTable[charLevel-1]) {
            charLevel++;
            power += charLevel;
            System.out.println(charLevel);
        }
    }
    public void successAttack(GameCharacter target, int amount) {

        target.takeDamage(this, (int) (amount * power));
        addExp(amount * 100);
    }
